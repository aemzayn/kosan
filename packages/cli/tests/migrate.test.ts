...
function makeMigration(name: string, sql: string): Migration {
  return {
    name,
    up: async (qi: QueryInterface) => { await qi.sequelize.query(sql); },
    down: async (qi: QueryInterface) => { void qi; },
  };
}
...
describe('runMigrationsForTenant', () => {
  ...
  it('returns up-to-date when there are no migrations', async () => {
    ...
    try {
      const result = await runMigrationsForTenant({
        tenant, sequelize, config: makeConfig(path.join(tmpDir, 'master.sqlite')), migrations: [],
      });
  ...
  it('runs pending migrations and returns migrated', async () => {
    ...
    const tenant = makeTenantConfig(dbPath);
    const migrations = [
      makeMigration('001-create-notes.js', 'CREATE TABLE IF NOT EXISTS notes (id INTEGER PRIMARY KEY)'),
    ];
  ...
  it('is idempotent — second run returns up-to-date', async () => {
    ...
    const migrations = [
      makeMigration('001-create-items.js', 'CREATE TABLE IF NOT EXISTS items (id INTEGER PRIMARY KEY)'),
    ];
  ...
  it('actually creates the table in the database', async () => {
    ...
    const config = makeConfig(path.join(tmpDir, 'master.sqlite'));
    const migrations = [
      makeMigration('001-create-events.js', 'CREATE TABLE IF NOT EXISTS events (id INTEGER PRIMARY KEY)'),
    ];

    const seq = openSqlite(dbPath);
    await runMigrationsForTenant({ tenant, sequelize: seq, config, migrations });
    const [rows] = await seq.query("SELECT name FROM sqlite_master WHERE type='table' AND name='events'");
  ...
  it('captures errors without throwing', async () => {
    ...
    const tenant = makeTenantConfig(dbPath);
    const migrations = [
      makeMigration('001-bad.js', 'THIS IS NOT VALID SQL !!!'),
    ];
  ...
  it('runs multiple migrations in order', async () => {
    ...
    const migrations: Migration[] = [
      { name: '001-first.js', up: async () => { order.push('first'); }, down: async () => {} },
      { name: '002-second.js', up: async () => { order.push('second'); }, down: async () => {} },
      { name: '003-third.js', up: async () => { order.push('third'); }, down: async () => {} },
    ];

    await runMigrationsForTenant({ tenant, sequelize, config: makeConfig(path.join(tmpDir, 'master.sqlite')), migrations });
...
describe('printResults', () => {
  it('renders a table without throwing', () => {
    ...
    const results: TenantMigrationResult[] = [
      { tenantId: 't1', slug: 'acme', outcome: 'migrated', applied: ['001.js', '002.js'], durationMs: 42 },
      { tenantId: 't2', slug: 'globex', outcome: 'up-to-date', applied: [], durationMs: 5 },
      { tenantId: 't3', slug: 'bad', outcome: 'failed', applied: [], error: new Error('db exploded'), durationMs: 1 },
